...
if ($adminRuangan > 0) {
    $permohonan1 = Permohonan_Rapat::where('status', 1)
        ->where('id_ruangrapat', Auth::user()->adminRuangan)
        ->count();
} else {
...
$ruangTerpakai2 = Permohonan_Rapat::where(function ($query) {
    ...
    ->whereDate('waktu_masuk', Carbon::now()->format('Y-m-d'))
...
<div class="row">
    ...
    <div class="col-sm-6 col-lg-3">
        <div class="panel panel-primary text-center">
            ...
            <div class="panel-body btn rapatDiterima">
                <h3 class=""><b><?php
                                echo $permohonan
                                ?></b></h3>
    ...
    <div class="col-sm-6 col-lg-3">
        <div class="panel panel-primary text-center">
            ...
            <div class="panel-body btn rapatDiajukan">
                <h3 class=""><b><?php
                                echo $permohonan1
                                ?></b></h3>
    ...
    <div class="col-sm-6 col-lg-3">
        <div class="panel panel-primary text-center">
            ...
            <div class="panel-body btn jumlahAdmin">
                <h3 class=""><b><?php
                                echo $pegawai
                                ?></b></h3>
    ...
    <!-- admin -->
    <div class="modal fade" id="modal_loop3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="false">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel-1">Admin</h4>
                </div>
                <div class="modal-body">
                    <div>
                        <table id="records_table" border="1" class="table table-striped table-bordered">
    ...
    <!-- rapatDiajukan -->
    <div class="modal fade" id="modal_loop4" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="false">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel-1">Rapat Diajukan</h4>
                </div>
                <div class="modal-body">
                    <div>
                        <table id="records_table" border="1" class="table table-striped table-bordered">
...
<script>
    ...
    // admin
    $(document).ready(function() {
        $('body').on('click', '.jumlahAdmin', function() {
            $.ajax({
                url: "/TampilAdmin",
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    ...
                    $.each(data, function(i, item) {
                        var nomor = i + 1;
                        trHTML += '<tr><td>' + nomor + '</td><td>' + item.nama + '</td></tr>';
                    });
                    //kode diatas method looping untuk table di javascript
                    $('#records_table tbody').html(trHTML);
                    $('#modal_loop3').modal('show');
                }
            });
        });
    });
    ...
    $(document).ready(function() {
        $('body').on('click', '.rapatDiajukan', function() {
            $.ajax({
                url: "/rapatDiajukan",
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    ...
                    $.each(data, function(i, item) {
                        var nomor = i + 1;
                        var tanggalPinjamDate = new Date(item.tanggal_pinjam);
                        var tanggalSelesaiDate = new Date(item.tanggal_selesai);

                        // Format the date and time
                        var formattedTanggalPinjam = formatDateAndTime(tanggalPinjamDate);
                        var formattedTanggalSelesai = formatDateAndTime(tanggalSelesaiDate);

                        trHTML += '<tr><td>' + nomor + '</td><td>' + item.nama_rapat + '</td><td>' + formattedTanggalPinjam + '</td><td>' + formattedTanggalSelesai + '</td><td>' + item.divisi.nama + '</td></tr>';
                    });
                    //kode diatas method looping untuk table di javascript
                    $('#records_table tbody').html(trHTML);
                    $('#modal_loop4').modal('show');
                }
            });
        });
    });
    ...
    // rapat DIterima
    $(document).ready(function() {
        $('body').on('click', '.rapatDiterima', function() {
            $.ajax({
                url: "/rapatDiterima",
                type: "GET",
                dataType: "JSON",
    ...
    // Function to format date and time as desired (e.g., "Senin, 27 Juli 2023 13:45")
