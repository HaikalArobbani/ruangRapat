...
class HomeController extends Controller
{
    ...
    {
        $id = $request->input('id');
        $data = DB::table('permohonan_rapat')->where('id_ruangrapat', $id)->where('status', 2)->get();
    ...
    { //cara penggunaaa
        ...
        // $jamKeluar = date('H:i', strtotime($waktuKeluar));
        // $jamMasuk = date('H:i', strtotime($waktuMasuk));
        // dd($jamMasuk);
        ...
        $selisihHari = $durasi->format('%a');
        $selisihJam = $durasi->format('%H');
        $selisihMenit = $durasi->format('%I');
        $selisihDetik = $durasi->format('%S');
    ...
    {
        // Ambil nilai 'idRapat' dari parameter permintaan AJAX
        $idRapat = $request->input('idRapat');

        // Lakukan query ke database menggunakan Laravel Query Builder
        $existingBookings = DB::table('permohonan_rapat')->where('id_ruangrapat', $idRapat)->where('status', 2)->get();
